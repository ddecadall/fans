...
bool SurfaceFlinger::authenticateSurfaceTexture(const sp<IGraphicBufferProducer>& bufferProducer) const
{
    Mutex::Autolock _l(mStateLock);
    return authenticateSurfaceTextureLocked(bufferProducer);
}

bool SurfaceFlinger::authenticateSurfaceTextureLocked(const sp<IGraphicBufferProducer>& bufferProducer) const 
{
...
status_t SurfaceFlinger::getDisplayConfigs(const sp<IBinder>& displayToken,Vector<DisplayInfo>* configs) 
{
...
status_t SurfaceFlinger::getDisplayColorModes(const sp<IBinder>& displayToken,Vector<ColorMode>* outColorModes) 
{
...
status_t SurfaceFlinger::getDisplayNativePrimaries(const sp<IBinder>& displayToken,ui::DisplayPrimaries &primaries) 
{
    if (!displayToken) 
    {
        return BAD_VALUE;
...
status_t SurfaceFlinger::getHdrCapabilities(const sp<IBinder>& displayToken,HdrCapabilities* outCapabilities) const 
{
    Mutex::Autolock _l(mStateLock);
...
status_t SurfaceFlinger::getDisplayedContentSamplingAttributes(const sp<IBinder>& displayToken,ui::PixelFormat* outFormat,ui::Dataspace* outDataspace,uint8_t* outComponentMask) const 
{
...
status_t SurfaceFlinger::setDisplayContentSamplingEnabled(const sp<IBinder>& displayToken,bool enable, uint8_t componentMask,uint64_t maxFrames) const 
{
    const auto display = getDisplayDevice(displayToken);
    if (!display || !display->getId()) 
    {
...
status_t SurfaceFlinger::getLayerDebugInfo(std::vector<LayerDebugInfo>* outLayers) const NO_THREAD_SAFETY_ANALYSIS 
{
        ...
        bool privileged) 
{
    ...
    {
        if (layer->setTransform(s.transform))
        { 
            flags |= eTraversalNeeded;
        }
    }
    ...
    {
        if (layer->setAcquireFence(s.acquireFence))
        {
            flags |= eTraversalNeeded;
        }
    }
    ...
    {
        if (layer->setDataspace(s.dataspace))
        {
            flags |= eTraversalNeeded;
        }
    }
    ...
    {
        if (layer->setHdrMetadata(s.hdrMetadata))
        {
            flags |= eTraversalNeeded;
        }
    }
    ...
    {
        if (layer->setSurfaceDamageRegion(s.surfaceDamageRegion))
        {
            flags |= eTraversalNeeded;
        }
    }
    ...
    {
        if (layer->setSidebandStream(s.sidebandStream))
        {
            flags |= eTraversalNeeded;
        }
    }
        ...
        float frameScale, bool childrenOnly) 
{
    ...
    public:
        ...
        const sp<const DisplayDevice> getDisplayDevice() const override 
        { 
            return nullptr; 
        }
        ...
        public:
            ...
            ~ReparentForDrawing() 
            { 
                oldParent->setChildrenDrawingParent(oldParent); 
            }
        };
